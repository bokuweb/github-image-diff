...
chrome.runtime.onMessage.addListener((msg, sender) => {
  ...
  const before = Uint8Array.from(
    Object.keys(msg.before.data).map(i => msg.before.data[i])
  );
  const after = Uint8Array.from(
    Object.keys(msg.after.data).map(i => msg.after.data[i])
  );
  q.push({
    index: msg.index,
    before: {
      data: before,
      width: msg.before.width
    },
    after: {
      data: after,
      width: msg.after.width
    }
  });
  if (!self.mod) return;
  run(sender.tab.id);
...
function run(id) {
  ...
  q.forEach(d => {
    ...
    console.timeEnd('a')
    chrome.tabs.sendMessage(id, { type: "diff", ...result, index: d.index });
  });
  q = [];
}
